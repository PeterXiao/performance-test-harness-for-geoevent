...
public class WebsocketServerEventConsumer extends ConsumerBase
{
	private WebsocketOutboundServlet	webSocketServlet;
	private Server										server;
	private int 											port = 5775;
	private String 										uri = "/ws-in";
	
	private static final String 			URI_SUFFIX = "/*";
	
	public WebsocketServerEventConsumer(int port)
	{
		this.port = port;
		setup(port);
	}
	...
	public void init(Config config) throws TestException
	{
		...
		try
		{
			int port = Integer.parseInt(config.getPropertyValue("port", "5775"));
			if( this.port != port )
			{
				this.port = port;
				shutdown();
	...
	public void validate() throws TestException
	{
		if (webSocketServlet == null)
			throw new TestException("Socket connection is not established. Please initialize WebsocketServerEventConsumer before it starts collecting diagnostics.");
	}
